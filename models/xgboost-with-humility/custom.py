...
from humility_rules import Humility
...
def init(code_dir):
    ...
    global humility
    g_code_dir = code_dir
    humility = Humility(os.path.join(code_dir, "humility_conf.yaml"))
...
def fit(
    ...
    **kwargs,):
    ...
    model = XGBClassifier(
        colsample_bylevel=0.2, max_depth=10, learning_rate=0.02, n_estimators=300
    )
...
def transform(data, model):
    ...
    pd.DataFrame
    """
    humility.check_input_humility(data)
...
def post_process(predictions, model):
    humility.check_output_humility(predictions)
    return predictions